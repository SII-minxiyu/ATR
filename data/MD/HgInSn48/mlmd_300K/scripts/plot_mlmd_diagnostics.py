...
def plot_temperature_energy(data: np.ndarray, out_path: Path) -> None:
    ...
    axes[0].plot(t, temp, color="#1f77b4", lw=1.5)
    axes[0].axhline(300, color="#555555", lw=1.0, ls="--", alpha=0.8)
    axes[0].set_ylabel("Temperature (K)")
    axes[0].set_title("CHGNet ML-MD 300 K NVT diagnostics")
    axes[0].grid(alpha=0.25)

    axes[1].plot(t, epot, label="Epot", color="#d62728", lw=1.4)
    axes[1].plot(t, etot, label="Etot", color="#2ca02c", lw=1.2)
...
def plot_potential_kinetic_force(data: np.ndarray, max_forces: np.ndarray, out_path: Path) -> None:
    ...
    axes[0].plot(t, epot, color="#d62728", lw=1.4)
    axes[0].set_ylabel("Epot (eV)")
    axes[0].set_title("Potential energy, kinetic energy, and maximum force")
    axes[0].grid(alpha=0.25)

    axes[1].plot(t, ekin, color="#ff7f0e", lw=1.4)
    axes[1].set_ylabel("Ekin (eV)")
    axes[1].grid(alpha=0.25)

    axes[2].plot(t, max_forces, color="#6a3d9a", lw=1.4)
...
def plot_min_distance(times: np.ndarray, mins: np.ndarray, out_path: Path) -> None:
    ...
    ax.plot(times, mins, color="#6a3d9a", lw=1.4)
    ...
    for col, frame_idx in enumerate(indices):
        ...
        for row, dims in enumerate(((0, 1), (0, 2))):
            ...
            for element in ("Hg", "In", "Sn"):
                mask = np.array([sym == element for sym in symbols])
                ax.scatter(
                    pos[mask, dims[0]],
                    pos[mask, dims[1]],
                    s=42,
...
def main() -> None:
    ...
    plot_temperature_energy(data, out_dir / "temperature_energy_10ps.png")
    plot_potential_kinetic_force(data, max_forces, out_dir / "epot_ekin_maxforce_10ps.png")
    plot_min_distance(data[:, 0], mins, out_dir / "min_pair_distance_10ps.png")
    plot_snapshots(frames, data, out_dir / "structure_snapshots_0_5_10ps.png")
    ...
    summary = {
        ...
        "max_force_eV_per_A_min": float(max_forces.min()),
