...
def min_distances(frames) -> np.ndarray:
    ...
    for atoms in frames:
        ...
        vals.append(float(distances[np.triu_indices(len(atoms), 1)].min()))
...
def plot_temperature_energy(data: np.ndarray, out_path: Path) -> None:
    ...
    axes[0].set_title("InSe48 CHGNet ML-MD 300 K NVT diagnostics")
...
def plot_potential_kinetic_force(data: np.ndarray, max_forces: np.ndarray, out_path: Path) -> None:
    ...
    axes[2].plot(t, max_forces, color="#6a3d9a", lw=1.1)
...
def plot_min_distance(times: np.ndarray, mins: np.ndarray, out_path: Path) -> None:
    ...
    ax.plot(times, mins, color="#6a3d9a", lw=1.1)
...
def plot_snapshots(frames, data: np.ndarray, out_path: Path) -> None:
    ...
    for col, frame_idx in enumerate(indices):
        ...
        for row, dims in enumerate(((0, 1), (0, 2))):
            ...
            ax.grid(alpha=0.15)
            ax.set_title(f"{times[frame_idx]:.1f} ps" if row == 0 else "")
...
def main() -> None:
    ...
    segments = parse_log_segments(run_dir / "mlmd.log")
    if not segments:
    ...
    stats = [
        stat_row("T_K", data[:, 4]),
        stat_row("Epot_eV", data[:, 2]),
        stat_row("Epot_atom_eV", epot_atom),
        stat_row("Ekin_eV", data[:, 3]),
        stat_row("Fmax_eV_per_A", max_forces),
        stat_row("dmin_A", mins),
    ]
    ...
    print(f"log_rows: {len(data)}")
    print(f"frames: {len(frames)}")
    print(f"time_ps_start: {data[0, 0]}")
    print(f"time_ps_end: {data[-1, 0]}")
